 ...
 *
 * Uso:
 *   node scripts/aplicar-folha-ponto.js <arquivo-rfp.xls> [--dry-run]
 *
...
// Mesma lista de src/lib/excel.ts — não aparecem no painel e ficam congelados.
const EXCLUIDOS = new Set([
  'leidiane souza', 'luiz henrique martins tavares', 'moises santiago',
  'cledjon dias dos santos', 'paulo rogerio santos', 'millena sthefany dos santos cruz',
  'anderson rosalvo rocha dos santos', 'samyra anchieta bispo',
  'cristielle pereira lima da silva', 'maria tatiane basto cardoso',
...
function lerFolhaPonto(arquivo) {
  ...
  for (let i = 0; i < rows.length; i++) {
    if (cell(i, 0) === 'DADOS DO COLABORADOR') { nome = cell(i + 1, 3); continue; }
    if (!nome) continue;
  ...
  if (nome) throw new Error(`bloco de "${nome}" terminou sem a linha de saldo`);
  return out;
}
...
function lerBancoHoras(rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (nome && typeof row[0] === 'string' && norm(row[0]).startsWith('total praticado hora excedente')) {
      out.push({ nome, saldo: String(row[12] ?? '').trim(), linhaTotal: i });
      nome = null;
    }
  }
  return out;
...
function main() {
  const arquivoRfp = process.argv[2];
  const dryRun = process.argv.includes('--dry-run');
  if (!arquivoRfp) {
    console.error('uso: node scripts/aplicar-folha-ponto.js <arquivo-rfp.xls> [--dry-run]');
    process.exit(1);
  }
  ...
  for (const n of novos) {
    ...
    const data = DATA_POR_GESTOR[gestor] || DATA_POR_GESTOR[n.nome] || DATA_PADRAO;
  ...
  const esperado = new Map(aplicados.map((a) => [norm(a.nome), a.para]));
  const depoisMap = new Map(depois.map((c) => [norm(c.nome), c]));
  for (const c of antes) {
    const d = depoisMap.get(norm(c.nome));
    if (!d) { erros.push(`sumiu: ${c.nome}`); continue; }
    const exp = esperado.has(norm(c.nome)) ? esperado.get(norm(c.nome)) : c.saldo;
    if (d.saldo !== exp) erros.push(`${c.nome}: esperado ${exp}, gravado ${d.saldo}`);
  }
